<template>
  <div class="flex flex-col min-h-screen bg-gray-50">
    <main class="flex-1 px-4 py-4 space-y-4 pb-28">
      ...
      <!-- ══════════════════════════════════════════
           LOADING SKELETON — mengganti spinner polos
      ══════════════════════════════════════════ -->
      <div v-if="initialLoading" class="space-y-3">
        ...
        >
          <!-- row atas: tanggal + badge status -->
          <div class="flex justify-between items-center mb-3">
            <div class="sk h-6 w-28 rounded-full"></div>
            <div class="sk h-6 w-20 rounded-full"></div>
          </div>

          <!-- plat & nama kendaraan -->
          <div class="bg-gray-50 rounded-xl p-3 border border-gray-200/50 space-y-2">
            <div class="sk h-6 w-36 rounded-lg"></div>
            <div class="sk h-4 w-48 rounded-lg"></div>
          </div>
        </div>
      </div>
...
<div class="bg-gradient-to-br from-gray-50 to-gray-100/50 rounded-xl p-3 border border-gray-200/50">
  ...
  <div
    v-if="job.label.license_plate || job.label.vehicle_name"
    class="flex items-center gap-3"
  >

    <!-- Thumbnail -->
<div
  class="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 bg-white border border-gray-200"
>
  <img
    v-if="job.label.display_image"
    :src="job.label.display_image"
    :alt="job.label.vehicle_name"
  ...
  >
    ...
    <div class="min-w-0 flex-1">
      ...
      <p class="font-mono font-bold text-lg text-gray-800 tracking-wider truncate">
        {{ job.label.license_plate || 'N/A' }}
      </p>

      <p class="font-bold text-gray-800 text-sm truncate">
        {{ job.label.vehicle_name || 'Kendaraan tidak tersedia' }}
      </p>
  ...
  <div v-else class="text-center py-2">
    <p class="text-gray-400 text-sm">
      Data kendaraan sudah tidak ada
    </p>
  </div>
        ...
        <div v-if="hasMoreData" ref="scrollTrigger" class="h-10 w-full"></div>

        <!-- Loading More — skeleton tambahan saat infinite scroll -->
        <div v-if="activeMenuLoading" class="space-y-3">
          <div
            v-for="n in 3"
            :key="'more-' + n"
            class="bg-white rounded-2xl p-3 shadow-sm border border-gray-100"
        ...
        <!-- Empty State -->
        <div v-if="displayedJobs.length === 0 && !initialLoading && !activeMenuLoading"
...
const displayedJobs  = computed(() => activeMenuItems.value);
...
const applyCompletedFilters = () => handleYearMonthFilter(selectedYear.value, selectedMonth.value);

const applyStatusFilter = () => {
  handleStatusFilter(selectedStatus.value === 'all' ? null : selectedStatus.value);
};

const goToJobDetail = (uuid: number) => router.push(`/jobs/${uuid}`);
...
const setupIntersectionObserver = () => {
  observer = new IntersectionObserver(
    (entries) => {
      ...
      if (entry?.isIntersecting && hasMoreData.value && !activeMenuLoading.value) loadMore();
    },
...
watch(searchQuery, (value) => handleSearch(value));
