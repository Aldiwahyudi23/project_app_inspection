...
// ─────────────────────────────────────────────────────────────
// FEATURE MATCH HELPER
// Cek apakah nama item cocok dengan salah satu fitur kendaraan
...
const sectionsWithProcessedItems = computed(() => {
  ...
  return formData.value.template.sections.map(section => ({
    ...
    items: section.items.map(item => {
      ...
      // ── Feature match: nama item cocok dengan fitur kendaraan ──
      ...
      if (isFeatureItem) {
        // Feature item: selalu tampil terlepas dari is_visible, damage, atau filter
        finalVisibility = true
      } else if (isForceVisible) {
        finalVisibility = true
      } else if (isTriggeredItem) {
        finalVisibility = isTriggered
      } else if (hasFilter) {
        finalVisibility = isVisibleByVehicle
      } else if (isDamageItem) {
        finalVisibility = false
      } else {
        finalVisibility = item.is_visible !== false
      }
      ...
      return {
        ...
        _isFeatureItem:      isFeatureItem,   // ← tambahan flag
      }
    })
  }))
})
