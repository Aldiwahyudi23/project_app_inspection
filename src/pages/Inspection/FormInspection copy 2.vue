        ...
        if (hasShowOption) {
          ...
          if (allImgs.length === 0) {
            ...
            const hasImages  = currentVal && typeof currentVal === 'object' && !Array.isArray(currentVal)
              ...
              : false
            if (hasImages) {
                // Re-sync image dari formValues ke store
                const serverImages = (currentVal.image as any[]).filter((img: any) => img?.id && img?.image_url)
          ...
          if (allImgs.length === 0) {
            ...
            // Guard: jangan reset kalau formValues sudah ada data
            // Ini terjadi saat store belum sync tapi localStorage sudah punya data
            if (currentIsArray && currentVal.length > 0) {
              // Re-sync dari formValues ke store agar ImageInput bisa tampilkan thumbnail
              imageStore.syncFromServer({
                serverImages: currentVal.filter((img: any) => img?.id && img?.image_url),
                sectionId:        itemId,
                itemId:           itemId,
                inspectionItemId: formItem.inspection_item_id,
                inspectionId:     inspectionId.value,
              })
              continue // jangan reset, jangan saveStorage
            }
            
            if (currentIsArray && currentVal.length === 0) {
              // Sudah kosong, tidak perlu apa-apa
