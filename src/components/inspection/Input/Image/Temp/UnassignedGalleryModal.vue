      ...
      <div v-else class="flex-1 overflow-y-auto overscroll-contain pb-28">
        <div class="grid grid-cols-3 gap-0.5 p-0.5">
          ...
          >
            ...
            <!-- FAILED — overlay retry + tombol X hapus -->
            <div
            ...
            <!-- Preview/zoom button — hanya saat done -->
            <button
              v-if="img.status === 'done' && targetItem"
            ...
            <!-- TOMBOL X HAPUS — muncul untuk status done dan failed -->
            <button
              v-if="(img.status === 'done' || img.status === 'failed') && !targetItem"
              class="absolute top-1.5 z-20 w-6 h-6 bg-black/60 backdrop-blur-sm rounded-full
                     flex items-center justify-center active:scale-90 transition-transform"
              :class="img.status === 'done' ? 'left-1.5' : 'right-1.5 top-1.5'"
              @click.stop="store.removeImage(img.localId)"
            >
              <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
