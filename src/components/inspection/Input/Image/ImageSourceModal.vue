<!-- components/inspection/Input/Image/ImageSourceModal.vue -->
<template>
  <Teleport to="body">
    <div
      v-if="show"
    ...
    >
      ...
      >
        ...
        <!-- Options: kamera + galeri selalu ada -->
        <div class="flex gap-3">
          ...
          <!-- Tombol "Foto Bebas" — muncul hanya jika ada temp images -->
          <button
            v-if="hasTempImages"
            class="flex-1 flex flex-col items-center justify-center rounded-xl p-3
                   transition-all hover:scale-105 active:scale-95 relative"
            :class="hasTempImages
              ? 'bg-orange-50 hover:bg-orange-100 border-2 border-orange-300'
              : 'bg-gray-100 hover:bg-gray-200'"
            @click="select('temp')"
          >
            <svg class="w-8 h-8 mb-2" :class="hasTempImages ? 'text-orange-500' : 'text-gray-700'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
            </svg>
            <span class="text-xs font-medium" :class="hasTempImages ? 'text-orange-700' : 'text-gray-900'">
              Foto Bebas
            </span>
            <!-- Badge count -->
            <div
              v-if="(tempCount ?? 0) > 0"
              class="absolute -top-1.5 -right-1.5 min-w-[20px] h-5 px-1 bg-orange-500 text-white
...
const select = (type: string) => emit('select', type)
